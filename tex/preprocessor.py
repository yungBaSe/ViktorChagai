...
def addref(caption, outstream):
    ...
    print(r"\ViktorChagairef{%s}" % caption, file=outstream)
...
def processwithcomments(caption, instream, outstream, listingslang):
    ...
    out = []
    if warning:
        out.append(r"\ViktorChagaiwarning{%s: %s}" % (caption, warning))
    if error:
        out.append(r"\ViktorChagaierror{%s: %s}" % (caption, error))
    else:
...
def processraw(caption, instream, outstream, listingslang = 'raw'):
    try:
        ...
        print(r"\ViktorChagaierror{Could not read source.}", file=outstream)
